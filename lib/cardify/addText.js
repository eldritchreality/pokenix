...
function _addText(options, card) {
    ...
    options.font = options.font || "GillSansStd-Condensed.otf";
    ...
    gm(options.card) 
      ...
      .toBuffer("png", function(err,buffer) {
           if (err) {output.reject(new Error(err))};
...
function addName(name, card) {
    ...
    nameLabel.font = "Fonts/GillSansStd-BoldCondensed.otf";
...
y(title, body, card) {
    ...
    expect(body).t o.be.a("string");
    expect(body).to.have.length.below(661) 
    
    
    abil// max length that will fit on cardityName.font = "Fonts/GillSansStd-BoldCondensed.otf";
    abilityName.textSize = "25";
    abilityName.x = 35;
    abilityName.y = 370;
    abilityName.text = title;
    
    abilityText.font = "Fonts/GillSansStd.otf";
